          ...
          <div class="border border-border rounded-lg divide-y divide-border">
            <div v-for="match in dateGroup.matches" :key="match.id" :id="`match-${match.id}`" class="px-4 py-3 space-y-1">
              ...
              <div class="flex items-center gap-3">
                <span class="flex-1 flex items-center justify-end gap-1 min-w-0"><span class="truncate text-sm font-medium">{{ teamName(match, 'home') }}</span><TeamFlag :team="teamName(match, 'home')" class="shrink-0" /></span>
                <template v-if="isEditing(match)">
                  ...
                  <span class="flex-1 flex items-center justify-start gap-1 min-w-0"><TeamFlag :team="teamName(match, 'away')" class="shrink-0" /><span class="truncate text-sm font-medium">{{ teamName(match, 'away') }}</span></span>
        ...
        <UCard>
          <div class="space-y-4">
            ...
            <div v-for="field in AWARD_FIELDS" :key="field.key" class="flex flex-col gap-2">
              ...
              <PlayerInput v-model="awards[field.key]" :players="allPlayers ?? []" />
            </div>
          </div>
        </UCard>
      </div>
